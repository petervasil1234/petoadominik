...
create view Pocet_jedalni as SELECT j.MESTO, COUNT(*) AS "POCET JEDALNI" FROM JEDALEN j GROUP BY j.MESTO; 

-- Najlacnejsia jedalen

create view najlacnejsia_jedalen as SELECT * FROM 
(SELECT j.MESTO, j.NAZOV, j.PERCENTO_NAVYSENIA|| ' %' as "Navysenie" FROM JEDALEN j ORDER BY j.PERCENTO_NAVYSENIA)
 WHERE ROWNUM <= 1;
 
-- Pocet muzov a zien v jedalniach

create view pocet_Muzov_a_Zien as SELECT j.NAZOV, z.POHLAVIE, COUNT(*) AS "POCET" FROM ZAMESTNANEC_JEDALNE z NATURAL JOIN JEDALEN j GROUP BY z.POHLAVIE, j.NAZOV ORDER BY 1;
...
create view priemerny_zisk as SELECT j.NAZOV, AVG(pr.ZISK) AS "PRIEMERNY ZISK" FROM PRODUKCIA_ZISKU pr NATURAL JOIN JEDALEN j GROUP BY j.NAZOV;
